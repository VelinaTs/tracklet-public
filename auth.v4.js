...
(function() {
    ...
    if (!token) {
      ...
      for (const ep of endpoints) {
        try {
          ...
          // Some servers just set cookies; others return a JSON payload
          ...
          // Re-check cookies after request (server may set readable token cookie)
          for (const name of cookieNames) {
            token = getCookie(name);
            if (token) break;
          }
  ...
  async function apiFetch(endpoint, options = {}) {
    ...
    if (isMutating(method)) {
      ...
      if (csrf) {
        // Attach common header names for broader compatibility
